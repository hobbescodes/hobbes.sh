...
function parseFrontmatter(raw: string): FrontmatterResult {
  ...
  for (const line of lines) {
    ...
    // Handle dates (YYYY-MM-DD format) - keep as string for consistency
    // No transformation needed for dates
...
export function getAllBlogPosts(): BlogPostMeta[] {
  const posts = Object.entries(blogFiles).map(([filePath, raw]) => {
    ...
    return {
      ...
      tags: (data.tags as string[]) || [],
      readingTime:
        (data.readingTime as string) || calculateReadingTime(content),
    };
  });
